...
. /blackbox/blackbox kafka-stdl check "test"
...
cat <<ASSERT | blackbox.expect.shell.success % {%s,%s,"Kafka"}
set -o errexit
{
  test -f /home/ubuntu/test/solve.assert
}
...
cat <<ASSERT | blackbox.expect.shell.success % {%s,%s,"Kafka topic"}
/usr/local/kafka/bin/kafka-topics.sh --bootstrap-server localhost:9092 --topic test --describe
ASSERT

cat <<ASSERT | blackbox.expect.shell.success % {%s,%s,"Kafka topic data"}
diff <(
  /usr/local/kafka/bin/kafka-console-consumer.sh --bootstrap-server localhost:9092 --topic test --from-beginning --timeout-ms 3000 2>/dev/null
) <(
  printf -- "test data\n"
)
ASSERT
