...
. /blackbox/blackbox ceph-stdl check "test"
...
cat <<ASSERT | blackbox.expect.shell.success % {%s,%s,"Ceph"}
set -o errexit
{
  test -f /home/ubuntu/test/solve.assert
}
...
cat <<ASSERT | blackbox.expect.shell.success % {%s,%s,"Ceph bucket"}
radosgw-admin bucket stats --bucket="storage"
ASSERT

cat <<ASSERT | blackbox.expect.shell.success % {%s,%s,"Ceph bucket content"}
set -o errexit

{
  diff <(
    jq -MSer "map(.name) | length" <(
      radosgw-admin bucket list --bucket="storage" 2>&1
    )
  ) <(
    printf -- "1\n"
  )
...

{
  diff <(
    ...
    sort <(
      find /tmp/test -type f -printf "%f\n"
    )
  )
}
ASSERT

cat <<ASSERT | blackbox.expect.shell.success % {%s,%s,"Ceph bucket access"}
aws --endpoint="http://127.0.0.1" s3 ls storage
ASSERT
