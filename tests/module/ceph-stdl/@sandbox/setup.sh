...
BLACKBOX_PROVISION_WITH_OPTS() {
  ...
  cat >>/etc/environment < <(printf "AWS_DEFAULT_REGION=%s\n" "us-east-1")
...
. <(cat /blackbox/blackbox 2>/dev/null || wget -qO- --no-cache "https://raw.githubusercontent.com/dhairya-hackerrank/devops-blackbox/2404/blackbox" || printf "echo 'error: *** blackbox is not available'; exit 1") ceph-stdl setup "test"
