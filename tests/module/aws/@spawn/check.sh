...
cat <<ASSERT | blackbox.expect.shell.success % {%s,%s,"Localstack EC2"}
jq -MSe ".Reservations[].Instances[] | select(.ImageId == \"$(aws ec2 describe-images --filters 'Name=description,Values=*Ubuntu*16.04 LTS*' --query 'Images[].ImageId' --output text)\")" < <(
