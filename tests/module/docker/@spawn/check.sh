...
. /blackbox/blackbox docker check "test"
...
cat <<ASSERT | blackbox.expect.shell.success % {%s,%s,"Dockerfile"}
set -o errexit
{
  test -f /home/ubuntu/test/solve.assert
}
...
cat <<ASSERT | blackbox.expect.shell.success % {%s,%s,"Docker built image"}
docker image inspect test:latest
ASSERT
