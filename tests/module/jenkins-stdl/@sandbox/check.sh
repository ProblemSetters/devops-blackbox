...
. /blackbox/blackbox jenkins-stdl check "test"
...
cat <<ASSERT | blackbox.expect.shell.success % {%s,%s,"Java"}
set -o errexit
{
  test -f /home/ubuntu/test/solve.assert
}
...
cat <<ASSERT | blackbox.expect.shell.success % {%s,%s,"Jenkins"}
set -o errexit
{
  test -f /home/ubuntu/test/solve.assert
}
...
cat <<ASSERT | blackbox.expect.shell.success % {%s,%s,"Jenkins CLI"}
set -o errexit
{
  test -f /home/ubuntu/test/solve.assert
}

java -jar /usr/local/bin/jenkins-cli.jar -version
ASSERT

cat <<ASSERT | blackbox.expect.shell.success % {%s,%s,"Jenkins 'JENKINS_URL' environment variable"}
set -o errexit
{
  test -f /home/ubuntu/test/solve.assert
}
...
diff <(
  printenv JENKINS_URL
) <(
  printf -- "http://localhost:8080\n"
)
ASSERT

cat <<ASSERT | blackbox.expect.shell.success % {%s,%s,"Jenkins 'JENKINS_USER_ID' environment variable"}
set -o errexit
{
  test -f /home/ubuntu/test/solve.assert
}
...
diff <(
  printenv JENKINS_USER_ID
) <(
  printf -- "admin\n"
)
ASSERT

cat <<ASSERT | blackbox.expect.shell.success % {%s,%s,"Jenkins 'JENKINS_API_TOKEN' environment variable"}
set -o errexit
{
  test -f /home/ubuntu/test/solve.assert
}
...
cat <<ASSERT | blackbox.expect.shell.success % {%s,%s,"Jenkins 'pipeline-model-definition' plugin"}
java -jar /usr/local/bin/jenkins-cli.jar list-plugins pipeline-model-definition
ASSERT

cat <<ASSERT | blackbox.expect.shell.success % {%s,%s,"Jenkins 'pipeline-model-definition' plugin"}
grep '^test stage is done\$' < <(
  java -jar /usr/local/bin/jenkins-cli.jar build test-job -s -v
)
ASSERT
